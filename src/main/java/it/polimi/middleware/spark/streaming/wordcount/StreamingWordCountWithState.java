...
public class StreamingWordCountWithState {
    public static void main(String[] args) {
        ...
        final JavaMapWithStateDStream<String, Integer, Integer, Tuple2<String, Integer>> state = sc
                ...
                // .window(Durations.seconds(5)) //when you put a word it remains in 5 windows, and gets counted 5 times since the state is kept
                .map(String::toLowerCase)
                .flatMap(line -> Arrays.asList(line.split(" ")).iterator())
                .mapToPair(word -> new Tuple2<>(word, 1))
                //always keeping the state, infinite window
